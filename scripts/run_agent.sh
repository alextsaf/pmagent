...
implement_until_green() {
  ...
  while [ "$attempt" -le "$IMPL_ATTEMPTS" ]; do
    ...
    if verify_gates; then
      ...
      # Stage everything, then unstage pmagent's own runtime artifacts (the engine
      # checkout is a nested repo that would otherwise commit as a submodule
      # gitlink). reset on an unstaged/ignored path is a harmless no-op.
      git add -A
      git reset -q -- .pmagent-engine .pmagent codemap .aider 2>/dev/null || true
